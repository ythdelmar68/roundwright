...
Roundlet's sole formal Orchestrator authority includes delegation to its verified persistent Worker of source edits, repository-prescribed validation, and ordinary atomic local commits strictly within the assigned issue scope and isolated worktree. These local implementation actions are performed on Roundlet's behalf, not as an independent orchestrator. The Orchestrator retains all GitHub mutations and branch/worktree lifecycle operations. A Worker receives no independent push, PR, merge, scheduling, policy activation, release, or publication authority; a Supervisor remains read-only. All existing prohibitions and host/platform gates remain effective. Candidate-authored authority changes remain inert until the reviewed, merged, owner-acknowledged transition below is complete.
