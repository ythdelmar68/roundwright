...
| Advisory profiles and grants do not create effective authority or a background service. Live host provisioning remains disabled and deferred: a Worker, Supervisor, or dependency-review adapter and each production wrapper fail closed without a matching independently verified capsule, before durable state writes, callbacks, checkpoints, or provider-session construction. The host must independently derive the expected immutable execution binding from repository, task, candidate, instance, host, deployment, epoch, fence, role, selected profile, actual request/attempt, and preflight; it must never use a capsule binding as its expected value. An admitted grant/instance consumes calls, duration, and tokens through a durable transactionally serialized ledger, which fails closed on malformed or unavailable persistence. Worker tool requests map only to the granted bounded-coding action and are denied before their local callback. Advisor and Interpreter stay non-dispatching. | `role_capability_policy.py` and Codex adapter entrypoints | Omitted, forged, wrong-role, stale, revoked, cross-seam, request/preflight drift, concurrent budget, ungranted-tool, binding-replay, and denial-before-effect tests using temporary-Git owner records. |
