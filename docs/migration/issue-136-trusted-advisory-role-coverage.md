...
| Capability use requires an existing owner/deployment read-back record, its independently pinned expectation, expiry, revocation, instance, and closed typed scope receipts; public output is only the granted intersection. A sealed runtime context owns Git identity and clock reads; the verified result and SDK mapping are immutable after read-back, and only the matching advisory seam may pass the shared pre-effect gate. | `role_capability_policy.py` v3 grant/admission receipts | Self-minted, stale, replay, drifted, post-read mutation, cross-seam, and capability-expansion adversarial tests. |
...
| Advisory profiles and grants do not create effective authority or a background service. Matching sealed capsules gate Worker, Supervisor, and dependency-review before a provider session; Advisor and Interpreter stay non-dispatching. | `role_capability_policy.py` and Codex adapter entrypoints | Disabled-without-grant, cross-seam, and denial-before-session tests. |
