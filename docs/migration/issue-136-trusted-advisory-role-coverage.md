...
| Advisory profiles and grants do not create authority or a background service. Every effect-capable Worker, Supervisor, dependency-review, provider-attempt, and shadow-sequence entrypoint requires a separately derived immutable execution expectation—repository, task, candidate, instance, host, deployment, epoch, fence, role, selected profile, actual request or attempt, and preflight—and compares it with the sealed execution before store creation, callbacks, checkpoints, provider-session construction, or native turns. The provider-attempt host reserves calls, duration, and tokens in one durable transactionally serialized grant/instance ledger before its attempt mutation or native effect; unavailable or malformed persistence denies it. Worker tool requests are reduced to the granted bounded-coding action and denied before the local callback. Every native Worker, Supervisor, and dependency-review bridge requires a sealed launch context that binds the verified profile and authoritative host repository cwd, injects only receipt/context-digest guidance, disables implicit instruction discovery, and is rechecked before client creation and every native turn or resumed session. Dependency review alone uses an empty ephemeral SDK workspace while retaining the sealed host cwd for verification. Advisor and Interpreter stay non-dispatching. | `role_capability_policy.py` and Codex adapter entrypoints | Omitted, forged, wrong-role, stale, revoked, cross-seam, request/preflight drift, concurrent budget, ungranted-tool, omitted expected execution, omitted sealed launch context, sealed-launch cwd/profile drift, binding-replay, and denial-before-effect tests using temporary-Git owner records. |
