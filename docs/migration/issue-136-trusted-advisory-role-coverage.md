...
| Capability use requires an existing owner/deployment read-back record, its independently pinned expectation, expiry, revocation, instance, and closed typed scope receipts; public output is only the granted intersection. The verified result and SDK mapping are immutable after read-back, and only the matching advisory seam may pass the shared pre-effect gate. | `role_capability_policy.py` v3 grant/admission receipts | Self-minted, stale, drifted, post-read mutation, cross-seam, and capability-expansion adversarial tests. |
