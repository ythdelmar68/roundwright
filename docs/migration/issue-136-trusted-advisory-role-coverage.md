...
| This milestone supplies a production-disabled contract, not a trusted host root. Every public effect-capable Worker, Supervisor/provider runtime, dependency-review, hosted/recovery adapter, and external-validation composition path fails at its first instruction before validation, state/store/Recorder construction, callbacks, checkpoints, provider-session construction, or native turns. The selected private external-validation harness preflight may construct a supplied fake dependency backend solely to validate its zero-session request envelope; it cannot select native discovery or production authority. Hermetic seams carry an immutable host-derived execution binding: `derive_for_effect` folds the exact request/attempt and preflight material into a distinct identity, which is compared at each fake-effect admission boundary so A-to-B replay, candidate drift, and expired admission deny before events. The SQLite ledger key includes that exact binding, serializes reconstruction with `BEGIN IMMEDIATE`, and reserves the selected effect's full calls/duration/tokens exposure before an attempt mutation or provider effect. Accepted-main guidance bytes, role-injected bytes, reviewed SDK-mapping identity, and the actual SDK cwd are sealed in `TrustedProviderLaunchContext`; the dependency bridge reseals its empty ephemeral workspace before SDK creation and turn dispatch. No qualified native discovery-off control exists, so every real/native bridge remains `NOT_ACTIVATED`; hermetic bridges pass only explicit injected bytes and cwd. Worker local tools still enforce exact path/process descriptors before each effect. Advisor and Interpreter stay non-dispatching. | `role_capability_policy.py`, `*_toolbox.py`, and `provider_attempt_runtime.py` | Omission, forged, wrong-role, stale, revoked, cross-seam, request/preflight A-to-B replay, exact-exposure and concurrent-budget, ungranted-tool, sealed launch bytes/cwd/mapping drift, tamper, and first-instruction zero-effect denial tests using temporary-Git hermetic records. |
