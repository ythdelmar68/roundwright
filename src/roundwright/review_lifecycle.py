...
class WorkerObjective:
    ...
    def __post_init__(self) -> None:
        if not all(_token(v) for v in (self.objective_id, self.task_id, self.provider_attempt_id, self.retry_identity)) or not _sha(self.candidate_sha) or not _digest(self.objective_digest):
...
class ReviewLifecycleStore:
    ...
    def start_objective(self, repository: RepositoryIdentity, identity: TaskIdentity, objective: WorkerObjective, *, lease: object) -> WorkerObjective:
        ...
        try:
            ...
            retry = _require_worker_attempt(connection, identity, objective.provider_attempt_id)
            ...
            row = connection.execute("SELECT task_id, candidate_sha, provider_attempt_id, retry_identity, objective_digest FROM worker_objective_records WHERE objective_id = ?", (objective.objective_id,)).fetchone()
            if row is None:
    ...
    def complete_objective(self, repository: RepositoryIdentity, identity: TaskIdentity, objective: WorkerObjective, *, lease: object, completion_digest: str) -> None:
        if not _digest(completion_digest):
            raise ReviewLifecycleError("objective completion is invalid")
        self._terminal_objective(repository, identity, objective, lease, ObjectiveState.COMPLETED, completion_digest)

    def cancel_objective(self, repository: RepositoryIdentity, identity: TaskIdentity, objective: WorkerObjective, *, lease: object, reason_digest: str) -> None:
        if not _digest(reason_digest):
            raise ReviewLifecycleError("objective cancellation is invalid")
        self._terminal_objective(repository, identity, objective, lease, ObjectiveState.CANCELLED, reason_digest)
    ...
    def queue_owner_command(self, repository: RepositoryIdentity, identity: TaskIdentity, command: OwnerCommand, *, lease: object) -> OwnerCommand:
        ...
        try:
            ...
            grant = connection.execute("SELECT owner_identity, command_scope, task_id, candidate_sha, state FROM owner_authority_grants WHERE grant_id = ?", (command.authority_grant_id,)).fetchone()
            if grant != (command.owner_identity, command.kind.value, identity.task_id, command.candidate_sha, "active"):
    ...
    def consume_owner_command(self, repository: RepositoryIdentity, identity: TaskIdentity, *, command_id: str, lease: object, result_digest: str) -> ReviewItemProjection:
        ...
        try:
            connection.execute("BEGIN IMMEDIATE")
            _require_lifecycle_authority(connection, identity, lease)
            row = connection.execute("SELECT command_kind, target_item_id, candidate_sha, state, result_digest FROM owner_command_records WHERE command_id = ? AND task_id = ?", (command_id, identity.task_id)).fetchone()
            if row is None:
    ...
    def _terminal_objective(self, repository: RepositoryIdentity, identity: TaskIdentity, objective: WorkerObjective, lease: object, state: ObjectiveState, digest: str) -> None:
        if objective.task_id != identity.task_id:
            raise ReviewLifecycleError("Worker objective task does not match its exact identity")
        connection = _open_writable_connection(repository)
        try:
...
def _require_worker_attempt(connection: sqlite3.Connection, identity: TaskIdentity, attempt_id: str) -> str:
    row = connection.execute("SELECT task_id, provider_role, attempt_number, state FROM provider_attempts WHERE attempt_id = ?", (attempt_id,)).fetchone()
    if row is None or row[0] != identity.task_id or row[1] != "worker" or row[3] not in {"dispatched", "completed", "accepted"}:
