...
MIGRATIONS = (
    ...
    Migration(
        28,
        (
            "ALTER TABLE diff_review_routes ADD COLUMN claimed_by_implementation_attempt_id TEXT",
        ),
        (
            ("diff_review_routes", "CREATE TABLE diff_review_routes (diff_review_attempt_id TEXT PRIMARY KEY REFERENCES diff_review_attempts(diff_review_attempt_id), task_id TEXT NOT NULL REFERENCES tasks(task_id), worker_thread_identity TEXT NOT NULL, finding_ids_json TEXT NOT NULL, consumed_by_implementation_attempt_id TEXT REFERENCES implementation_attempts(implementation_attempt_id), claimed_by_implementation_attempt_id TEXT)"),
        ),
    ),
    Migration(
        29,
        (
            "ALTER TABLE diff_review_routes ADD COLUMN claimed_provider_attempt_id TEXT",
            "ALTER TABLE diff_review_routes ADD COLUMN claimed_external_turn_identity TEXT",
        ),
        (
    ...
    Migration(
        39,
        ("ALTER TABLE accepted_provider_reviews ADD COLUMN within_round_attempt INTEGER NOT NULL DEFAULT 0",),
        (("accepted_provider_reviews", "CREATE TABLE accepted_provider_reviews (accepted_review_identity TEXT PRIMARY KEY, task_id TEXT NOT NULL REFERENCES tasks(task_id), attempt_id TEXT NOT NULL UNIQUE REFERENCES provider_attempts(attempt_id), completion_evidence_fingerprint TEXT NOT NULL, configuration_schema_version TEXT NOT NULL DEFAULT '', configuration_digest TEXT NOT NULL DEFAULT '', worker_profile_identity TEXT NOT NULL DEFAULT '', supervisor_profile_identities TEXT NOT NULL DEFAULT '', selected_profile_identity TEXT NOT NULL DEFAULT '', within_round_attempt INTEGER NOT NULL DEFAULT 0)"),),
    ),
    Migration(
        40,
        (
            "ALTER TABLE diff_review_attempts ADD COLUMN review_round INTEGER NOT NULL DEFAULT 0",
            "ALTER TABLE diff_review_attempts ADD COLUMN review_mode TEXT NOT NULL DEFAULT ''",
            "ALTER TABLE diff_review_attempts ADD COLUMN review_max_rounds INTEGER NOT NULL DEFAULT 0",
            "ALTER TABLE diff_review_attempts ADD COLUMN review_on_final_findings TEXT NOT NULL DEFAULT ''",
            "ALTER TABLE diff_review_attempts ADD COLUMN review_policy_digest TEXT NOT NULL DEFAULT ''",
        ),
    ...
    Migration(
        49,
        (
            # Epoch zero denotes rows written before formal review epochs were
            # durable.  It is deliberately not inferred from a later runtime:
            # legacy evidence remains byte-for-byte its original history.
            "ALTER TABLE diff_review_attempts ADD COLUMN review_epoch INTEGER NOT NULL DEFAULT 0",
            "ALTER TABLE accepted_provider_reviews ADD COLUMN review_epoch INTEGER NOT NULL DEFAULT 0",
        ),
        (
            ("diff_review_attempts", "CREATE TABLE diff_review_attempts (diff_review_attempt_id TEXT PRIMARY KEY, task_id TEXT NOT NULL REFERENCES tasks(task_id), implementation_attempt_id TEXT NOT NULL REFERENCES implementation_attempts(implementation_attempt_id), provider_attempt_id TEXT NOT NULL UNIQUE REFERENCES provider_attempts(attempt_id), supervisor_session_identity TEXT NOT NULL UNIQUE, external_turn_identity TEXT NOT NULL, message_identity TEXT NOT NULL, base_sha TEXT NOT NULL, candidate_sha TEXT NOT NULL, input_digest TEXT NOT NULL, state TEXT NOT NULL CHECK(state IN ('dispatched', 'recorded', 'accepted')), created_at INTEGER NOT NULL CHECK(created_at > 0), verification_digest TEXT NOT NULL DEFAULT '', accepted_review_identity TEXT, within_round_attempt INTEGER NOT NULL DEFAULT 0, selected_profile_identity TEXT NOT NULL DEFAULT '', review_round INTEGER NOT NULL DEFAULT 0, review_mode TEXT NOT NULL DEFAULT '', review_max_rounds INTEGER NOT NULL DEFAULT 0, review_on_final_findings TEXT NOT NULL DEFAULT '', review_policy_digest TEXT NOT NULL DEFAULT '', review_complete_rounds INTEGER NOT NULL DEFAULT 0, review_max_supervisor_attempts_per_round INTEGER NOT NULL DEFAULT 0, review_epoch INTEGER NOT NULL DEFAULT 0)"),
            ("accepted_provider_reviews", "CREATE TABLE accepted_provider_reviews (accepted_review_identity TEXT PRIMARY KEY, task_id TEXT NOT NULL REFERENCES tasks(task_id), attempt_id TEXT NOT NULL UNIQUE REFERENCES provider_attempts(attempt_id), completion_evidence_fingerprint TEXT NOT NULL, configuration_schema_version TEXT NOT NULL DEFAULT '', configuration_digest TEXT NOT NULL DEFAULT '', worker_profile_identity TEXT NOT NULL DEFAULT '', supervisor_profile_identities TEXT NOT NULL DEFAULT '', selected_profile_identity TEXT NOT NULL DEFAULT '', within_round_attempt INTEGER NOT NULL DEFAULT 0, review_complete_rounds INTEGER NOT NULL DEFAULT 0, review_max_rounds INTEGER NOT NULL DEFAULT 0, review_max_supervisor_attempts_per_round INTEGER NOT NULL DEFAULT 0, review_on_final_findings TEXT NOT NULL DEFAULT '', review_policy_digest TEXT NOT NULL DEFAULT '', review_epoch INTEGER NOT NULL DEFAULT 0)"),
        ),
    ),
    Migration(
        50,
        (
            "CREATE TABLE dependency_review_subsets (snapshot_id TEXT PRIMARY KEY, task_id TEXT NOT NULL REFERENCES tasks(task_id), source_digest TEXT NOT NULL, candidate_sha TEXT NOT NULL, policy_digest TEXT NOT NULL, configuration_digest TEXT NOT NULL, boundary_digest TEXT NOT NULL, creation_reason TEXT NOT NULL, content_digest TEXT NOT NULL, member_count INTEGER NOT NULL CHECK(member_count > 0))",
            "CREATE TABLE dependency_review_subset_members (snapshot_id TEXT NOT NULL REFERENCES dependency_review_subsets(snapshot_id), member_id TEXT NOT NULL, member_fingerprint TEXT NOT NULL, content_digest TEXT NOT NULL, PRIMARY KEY(snapshot_id, member_id), UNIQUE(snapshot_id, member_fingerprint))",
            "CREATE TABLE dependency_review_attempts (attempt_id TEXT PRIMARY KEY, task_id TEXT NOT NULL REFERENCES tasks(task_id), snapshot_id TEXT NOT NULL UNIQUE REFERENCES dependency_review_subsets(snapshot_id), profile_identity TEXT NOT NULL, configuration_digest TEXT NOT NULL, input_digest TEXT NOT NULL, supersedes_attempt_id TEXT REFERENCES dependency_review_attempts(attempt_id), state TEXT NOT NULL CHECK(state IN ('prepared', 'accepted', 'invalid', 'blocked')), UNIQUE(task_id, input_digest))",
            "CREATE TABLE dependency_review_proposals (proposal_id TEXT PRIMARY KEY, attempt_id TEXT NOT NULL UNIQUE REFERENCES dependency_review_attempts(attempt_id), proposal_digest TEXT NOT NULL, requested_disposition TEXT NOT NULL CHECK(requested_disposition IN ('auto-activate', 'owner-review', 'reject')), owner_route TEXT NOT NULL, UNIQUE(attempt_id, proposal_digest))",
            "CREATE TABLE dependency_review_proposal_edges (proposal_id TEXT NOT NULL REFERENCES dependency_review_proposals(proposal_id), ordinal INTEGER NOT NULL CHECK(ordinal >= 0), edge_kind TEXT NOT NULL CHECK(edge_kind IN ('explicit', 'policy-derived', 'semantic-inferred')), direction TEXT NOT NULL CHECK(direction IN ('depends-on', 'blocks')), subject_member_id TEXT NOT NULL, object_member_id TEXT NOT NULL, rationale_digest TEXT NOT NULL, confidence TEXT NOT NULL CHECK(confidence IN ('high', 'medium', 'low')), conflicts_digest TEXT NOT NULL, PRIMARY KEY(proposal_id, ordinal), UNIQUE(proposal_id, subject_member_id, object_member_id, edge_kind, direction))",
            "CREATE TABLE dependency_review_validation_outcomes (attempt_id TEXT PRIMARY KEY REFERENCES dependency_review_attempts(attempt_id), outcome TEXT NOT NULL CHECK(outcome IN ('accepted', 'invalid', 'blocked')), reason_code TEXT NOT NULL, output_digest TEXT NOT NULL, owner_route TEXT NOT NULL)",
        ),
        (
            ("dependency_review_subsets", "CREATE TABLE dependency_review_subsets (snapshot_id TEXT PRIMARY KEY, task_id TEXT NOT NULL REFERENCES tasks(task_id), source_digest TEXT NOT NULL, candidate_sha TEXT NOT NULL, policy_digest TEXT NOT NULL, configuration_digest TEXT NOT NULL, boundary_digest TEXT NOT NULL, creation_reason TEXT NOT NULL, content_digest TEXT NOT NULL, member_count INTEGER NOT NULL CHECK(member_count > 0))"),
            ("dependency_review_subset_members", "CREATE TABLE dependency_review_subset_members (snapshot_id TEXT NOT NULL REFERENCES dependency_review_subsets(snapshot_id), member_id TEXT NOT NULL, member_fingerprint TEXT NOT NULL, content_digest TEXT NOT NULL, PRIMARY KEY(snapshot_id, member_id), UNIQUE(snapshot_id, member_fingerprint))"),
            ("dependency_review_attempts", "CREATE TABLE dependency_review_attempts (attempt_id TEXT PRIMARY KEY, task_id TEXT NOT NULL REFERENCES tasks(task_id), snapshot_id TEXT NOT NULL UNIQUE REFERENCES dependency_review_subsets(snapshot_id), profile_identity TEXT NOT NULL, configuration_digest TEXT NOT NULL, input_digest TEXT NOT NULL, supersedes_attempt_id TEXT REFERENCES dependency_review_attempts(attempt_id), state TEXT NOT NULL CHECK(state IN ('prepared', 'accepted', 'invalid', 'blocked')), UNIQUE(task_id, input_digest))"),
            ("dependency_review_proposals", "CREATE TABLE dependency_review_proposals (proposal_id TEXT PRIMARY KEY, attempt_id TEXT NOT NULL UNIQUE REFERENCES dependency_review_attempts(attempt_id), proposal_digest TEXT NOT NULL, requested_disposition TEXT NOT NULL CHECK(requested_disposition IN ('auto-activate', 'owner-review', 'reject')), owner_route TEXT NOT NULL, UNIQUE(attempt_id, proposal_digest))"),
    ...
    Migration(
        57,
        (
            "ALTER TABLE gate_contexts ADD COLUMN terminal_dependency_binding TEXT NOT NULL DEFAULT ''",
        ),
        (
            ("gate_contexts", "CREATE TABLE \"gate_contexts\" (task_id TEXT NOT NULL REFERENCES tasks(task_id), candidate_sha TEXT NOT NULL, source_count INTEGER NOT NULL CHECK(source_count > 0), isolated_local_task INTEGER NOT NULL CHECK(isolated_local_task IN (0, 1)), policy_digest TEXT NOT NULL, receipt_fingerprint TEXT NOT NULL, policy_activated_at TEXT NOT NULL DEFAULT '', configuration_schema_version TEXT NOT NULL DEFAULT '', configuration_digest TEXT NOT NULL DEFAULT '', worker_profile_identity TEXT NOT NULL DEFAULT '', supervisor_profile_identities TEXT NOT NULL DEFAULT '', selected_supervisor_profile_identity TEXT NOT NULL DEFAULT '', review_complete_rounds INTEGER NOT NULL DEFAULT 0, review_max_rounds INTEGER NOT NULL DEFAULT 0, review_max_supervisor_attempts_per_round INTEGER NOT NULL DEFAULT 0, review_on_final_findings TEXT NOT NULL DEFAULT '', review_policy_digest TEXT NOT NULL DEFAULT '', dependency_graph_version_id TEXT NOT NULL DEFAULT '', dependency_graph_decision_digest TEXT NOT NULL DEFAULT '', terminal_dependency_binding TEXT NOT NULL DEFAULT '', PRIMARY KEY(task_id, candidate_sha))"),
        ),
    ),
    Migration(
        58,
        (
            "CREATE TABLE structured_review_items (item_id TEXT PRIMARY KEY, task_id TEXT NOT NULL REFERENCES tasks(task_id), review_identity TEXT NOT NULL, candidate_sha TEXT NOT NULL, item_kind TEXT NOT NULL CHECK(item_kind IN ('pass-follow-up', 'finding')), source_kind TEXT NOT NULL CHECK(source_kind IN ('accepted-review', 'supervisor-finding')), content_digest TEXT NOT NULL, destination TEXT NOT NULL, blocker_state TEXT NOT NULL CHECK(blocker_state IN ('blocking', 'non-blocking')), verification_state TEXT NOT NULL CHECK(verification_state IN ('pending', 'verified', 'waived')), disposition TEXT NOT NULL CHECK(disposition IN ('pending', 'resolved', 'waived')), created_at INTEGER NOT NULL CHECK(created_at > 0), resolved_command_id TEXT UNIQUE, UNIQUE(task_id, candidate_sha, review_identity, item_kind, content_digest))",
            "CREATE TABLE worker_objectives (objective_id TEXT PRIMARY KEY, task_id TEXT NOT NULL REFERENCES tasks(task_id), candidate_sha TEXT NOT NULL, provider_attempt_id TEXT NOT NULL, retry_identity TEXT NOT NULL, objective_digest TEXT NOT NULL, state TEXT NOT NULL CHECK(state IN ('active', 'cancelled', 'completed')), completion_digest TEXT, terminal_reason_digest TEXT, UNIQUE(task_id, candidate_sha, provider_attempt_id), UNIQUE(task_id, candidate_sha, objective_digest), CHECK((state = 'active' AND completion_digest IS NULL AND terminal_reason_digest IS NULL) OR (state = 'cancelled' AND completion_digest IS NULL AND terminal_reason_digest IS NOT NULL) OR (state = 'completed' AND completion_digest IS NOT NULL AND terminal_reason_digest IS NULL)))",
            "CREATE TABLE owner_commands (command_id TEXT PRIMARY KEY, task_id TEXT NOT NULL REFERENCES tasks(task_id), command_kind TEXT NOT NULL CHECK(command_kind IN ('resolve-review-item', 'waive-review-item')), owner_source TEXT NOT NULL CHECK(owner_source = 'allowlisted-owner'), target_item_id TEXT NOT NULL REFERENCES structured_review_items(item_id), candidate_sha TEXT NOT NULL, command_digest TEXT NOT NULL, idempotency_key TEXT NOT NULL UNIQUE, state TEXT NOT NULL CHECK(state IN ('pending', 'consumed')), result_digest TEXT, CHECK((state = 'pending' AND result_digest IS NULL) OR (state = 'consumed' AND result_digest IS NOT NULL)))",
        ),
        (
            ("structured_review_items", "CREATE TABLE structured_review_items (item_id TEXT PRIMARY KEY, task_id TEXT NOT NULL REFERENCES tasks(task_id), review_identity TEXT NOT NULL, candidate_sha TEXT NOT NULL, item_kind TEXT NOT NULL CHECK(item_kind IN ('pass-follow-up', 'finding')), source_kind TEXT NOT NULL CHECK(source_kind IN ('accepted-review', 'supervisor-finding')), content_digest TEXT NOT NULL, destination TEXT NOT NULL, blocker_state TEXT NOT NULL CHECK(blocker_state IN ('blocking', 'non-blocking')), verification_state TEXT NOT NULL CHECK(verification_state IN ('pending', 'verified', 'waived')), disposition TEXT NOT NULL CHECK(disposition IN ('pending', 'resolved', 'waived')), created_at INTEGER NOT NULL CHECK(created_at > 0), resolved_command_id TEXT UNIQUE, UNIQUE(task_id, candidate_sha, review_identity, item_kind, content_digest))"),
            ("worker_objectives", "CREATE TABLE worker_objectives (objective_id TEXT PRIMARY KEY, task_id TEXT NOT NULL REFERENCES tasks(task_id), candidate_sha TEXT NOT NULL, provider_attempt_id TEXT NOT NULL, retry_identity TEXT NOT NULL, objective_digest TEXT NOT NULL, state TEXT NOT NULL CHECK(state IN ('active', 'cancelled', 'completed')), completion_digest TEXT, terminal_reason_digest TEXT, UNIQUE(task_id, candidate_sha, provider_attempt_id), UNIQUE(task_id, candidate_sha, objective_digest), CHECK((state = 'active' AND completion_digest IS NULL AND terminal_reason_digest IS NULL) OR (state = 'cancelled' AND completion_digest IS NULL AND terminal_reason_digest IS NOT NULL) OR (state = 'completed' AND completion_digest IS NOT NULL AND terminal_reason_digest IS NULL)))"),
            ("owner_commands", "CREATE TABLE owner_commands (command_id TEXT PRIMARY KEY, task_id TEXT NOT NULL REFERENCES tasks(task_id), command_kind TEXT NOT NULL CHECK(command_kind IN ('resolve-review-item', 'waive-review-item')), owner_source TEXT NOT NULL CHECK(owner_source = 'allowlisted-owner'), target_item_id TEXT NOT NULL REFERENCES structured_review_items(item_id), candidate_sha TEXT NOT NULL, command_digest TEXT NOT NULL, idempotency_key TEXT NOT NULL UNIQUE, state TEXT NOT NULL CHECK(state IN ('pending', 'consumed')), result_digest TEXT, CHECK((state = 'pending' AND result_digest IS NULL) OR (state = 'consumed' AND result_digest IS NOT NULL)))"),
        ),
    ),
    Migration(
        59,
        (
            "CREATE TABLE review_item_records (item_id TEXT PRIMARY KEY, task_id TEXT NOT NULL REFERENCES tasks(task_id), review_identity TEXT NOT NULL, candidate_sha TEXT NOT NULL, item_kind TEXT NOT NULL CHECK(item_kind IN ('pass-follow-up', 'finding')), source_kind TEXT NOT NULL CHECK(source_kind IN ('accepted-review', 'supervisor-finding')), source_attempt_id TEXT NOT NULL REFERENCES provider_attempts(attempt_id), source_owner_identity TEXT NOT NULL, content_digest TEXT NOT NULL, destination TEXT NOT NULL, blocker_state TEXT NOT NULL CHECK(blocker_state IN ('blocking', 'non-blocking')), verification_state TEXT NOT NULL CHECK(verification_state IN ('pending', 'verified', 'waived')), disposition TEXT NOT NULL CHECK(disposition IN ('pending', 'resolved', 'waived')), created_at INTEGER NOT NULL CHECK(created_at > 0), resolved_command_id TEXT UNIQUE, UNIQUE(task_id, candidate_sha, review_identity, item_kind, content_digest))",
            "CREATE TABLE review_item_provenance (item_id TEXT PRIMARY KEY REFERENCES review_item_records(item_id), task_id TEXT NOT NULL REFERENCES tasks(task_id), candidate_sha TEXT NOT NULL, review_identity TEXT NOT NULL, source_kind TEXT NOT NULL CHECK(source_kind IN ('accepted-review', 'supervisor-finding')), source_attempt_id TEXT NOT NULL REFERENCES provider_attempts(attempt_id), source_owner_identity TEXT NOT NULL, UNIQUE(task_id, candidate_sha, review_identity, source_kind, source_attempt_id))",
            "CREATE TABLE worker_objective_records (objective_id TEXT PRIMARY KEY, task_id TEXT NOT NULL REFERENCES tasks(task_id), candidate_sha TEXT NOT NULL, provider_attempt_id TEXT NOT NULL UNIQUE REFERENCES provider_attempts(attempt_id), retry_identity TEXT NOT NULL UNIQUE, objective_digest TEXT NOT NULL, state TEXT NOT NULL CHECK(state IN ('active', 'cancelled', 'completed')), completion_digest TEXT, terminal_reason_digest TEXT, UNIQUE(task_id, candidate_sha, objective_digest), CHECK((state = 'active' AND completion_digest IS NULL AND terminal_reason_digest IS NULL) OR (state = 'cancelled' AND completion_digest IS NULL AND terminal_reason_digest IS NOT NULL) OR (state = 'completed' AND completion_digest IS NOT NULL AND terminal_reason_digest IS NULL)))",
            "CREATE TABLE owner_authority_grants (grant_id TEXT PRIMARY KEY, owner_identity TEXT NOT NULL, command_scope TEXT NOT NULL CHECK(command_scope IN ('resolve-review-item', 'waive-review-item')), task_id TEXT NOT NULL REFERENCES tasks(task_id), candidate_sha TEXT NOT NULL, authority_digest TEXT NOT NULL, state TEXT NOT NULL CHECK(state = 'active'), UNIQUE(owner_identity, command_scope, task_id, candidate_sha))",
        ...
        (
            ("review_item_records", "CREATE TABLE review_item_records (item_id TEXT PRIMARY KEY, task_id TEXT NOT NULL REFERENCES tasks(task_id), review_identity TEXT NOT NULL, candidate_sha TEXT NOT NULL, item_kind TEXT NOT NULL CHECK(item_kind IN ('pass-follow-up', 'finding')), source_kind TEXT NOT NULL CHECK(source_kind IN ('accepted-review', 'supervisor-finding')), source_attempt_id TEXT NOT NULL REFERENCES provider_attempts(attempt_id), source_owner_identity TEXT NOT NULL, content_digest TEXT NOT NULL, destination TEXT NOT NULL, blocker_state TEXT NOT NULL CHECK(blocker_state IN ('blocking', 'non-blocking')), verification_state TEXT NOT NULL CHECK(verification_state IN ('pending', 'verified', 'waived')), disposition TEXT NOT NULL CHECK(disposition IN ('pending', 'resolved', 'waived')), created_at INTEGER NOT NULL CHECK(created_at > 0), resolved_command_id TEXT UNIQUE, UNIQUE(task_id, candidate_sha, review_identity, item_kind, content_digest))"),
            ("review_item_provenance", "CREATE TABLE review_item_provenance (item_id TEXT PRIMARY KEY REFERENCES review_item_records(item_id), task_id TEXT NOT NULL REFERENCES tasks(task_id), candidate_sha TEXT NOT NULL, review_identity TEXT NOT NULL, source_kind TEXT NOT NULL CHECK(source_kind IN ('accepted-review', 'supervisor-finding')), source_attempt_id TEXT NOT NULL REFERENCES provider_attempts(attempt_id), source_owner_identity TEXT NOT NULL, UNIQUE(task_id, candidate_sha, review_identity, source_kind, source_attempt_id))"),
            ("worker_objective_records", "CREATE TABLE worker_objective_records (objective_id TEXT PRIMARY KEY, task_id TEXT NOT NULL REFERENCES tasks(task_id), candidate_sha TEXT NOT NULL, provider_attempt_id TEXT NOT NULL UNIQUE REFERENCES provider_attempts(attempt_id), retry_identity TEXT NOT NULL UNIQUE, objective_digest TEXT NOT NULL, state TEXT NOT NULL CHECK(state IN ('active', 'cancelled', 'completed')), completion_digest TEXT, terminal_reason_digest TEXT, UNIQUE(task_id, candidate_sha, objective_digest), CHECK((state = 'active' AND completion_digest IS NULL AND terminal_reason_digest IS NULL) OR (state = 'cancelled' AND completion_digest IS NULL AND terminal_reason_digest IS NOT NULL) OR (state = 'completed' AND completion_digest IS NOT NULL AND terminal_reason_digest IS NULL)))"),
            ("owner_authority_grants", "CREATE TABLE owner_authority_grants (grant_id TEXT PRIMARY KEY, owner_identity TEXT NOT NULL, command_scope TEXT NOT NULL CHECK(command_scope IN ('resolve-review-item', 'waive-review-item')), task_id TEXT NOT NULL REFERENCES tasks(task_id), candidate_sha TEXT NOT NULL, authority_digest TEXT NOT NULL, state TEXT NOT NULL CHECK(state = 'active'), UNIQUE(owner_identity, command_scope, task_id, candidate_sha))"),
    ...
    Migration(
        63,
        (
            "ALTER TABLE worker_objective_records RENAME TO worker_objective_records_legacy",
            "CREATE TABLE phase5_objective_upgrade_guard (value INTEGER NOT NULL CHECK(value = 1))",
            "INSERT INTO phase5_objective_upgrade_guard(value) SELECT 0 FROM worker_objective_records_legacy AS objectives LEFT JOIN provider_attempts AS attempts ON attempts.attempt_id = objectives.provider_attempt_id AND attempts.task_id = objectives.task_id LEFT JOIN implementation_attempts AS implementations ON implementations.provider_attempt_id = objectives.provider_attempt_id AND implementations.task_id = objectives.task_id LEFT JOIN implementation_candidates AS candidates ON candidates.implementation_attempt_id = implementations.implementation_attempt_id AND candidates.task_id = objectives.task_id LEFT JOIN provider_completion_outputs AS outputs ON outputs.attempt_id = objectives.provider_attempt_id WHERE attempts.attempt_id IS NULL OR implementations.implementation_attempt_id IS NULL OR (objectives.state = 'active' AND (attempts.state != 'dispatched' OR implementations.state != 'dispatched')) OR (objectives.state = 'completed' AND (attempts.state != 'completed' OR implementations.state != 'recorded' OR candidates.candidate_sha != objectives.candidate_sha OR candidates.completion_evidence_fingerprint != attempts.completion_evidence_fingerprint OR candidates.content_digest != outputs.output_fingerprint))",
            "INSERT INTO phase5_objective_upgrade_guard(value) SELECT 0 FROM worker_objectives AS objectives LEFT JOIN provider_attempts AS attempts ON attempts.attempt_id = objectives.provider_attempt_id AND attempts.task_id = objectives.task_id LEFT JOIN implementation_attempts AS implementations ON implementations.provider_attempt_id = objectives.provider_attempt_id AND implementations.task_id = objectives.task_id LEFT JOIN implementation_candidates AS candidates ON candidates.implementation_attempt_id = implementations.implementation_attempt_id AND candidates.task_id = objectives.task_id LEFT JOIN provider_completion_outputs AS outputs ON outputs.attempt_id = objectives.provider_attempt_id WHERE attempts.attempt_id IS NULL OR implementations.implementation_attempt_id IS NULL OR (objectives.state = 'active' AND (attempts.state != 'dispatched' OR implementations.state != 'dispatched')) OR (objectives.state = 'completed' AND (attempts.state != 'completed' OR implementations.state != 'recorded' OR candidates.candidate_sha != objectives.candidate_sha OR candidates.completion_evidence_fingerprint != attempts.completion_evidence_fingerprint OR candidates.content_digest != outputs.output_fingerprint))",
            ...
            "INSERT INTO worker_objective_records(objective_id, task_id, dispatch_sha, provider_attempt_id, retry_identity, objective_digest, state, candidate_sha, completion_evidence_fingerprint, accepted_result_identity, terminal_reason_digest) SELECT objectives.objective_id, objectives.task_id, CASE WHEN implementations.repair_candidate_sha IS NULL THEN tasks.base_sha ELSE implementations.repair_candidate_sha END, objectives.provider_attempt_id, objectives.retry_identity, objectives.objective_digest, objectives.state, CASE WHEN objectives.state = 'completed' THEN candidates.candidate_sha ELSE NULL END, CASE WHEN objectives.state = 'completed' THEN attempts.completion_evidence_fingerprint ELSE NULL END, CASE WHEN objectives.state = 'completed' THEN outputs.output_fingerprint ELSE NULL END, CASE WHEN objectives.state = 'cancelled' THEN objectives.terminal_reason_digest ELSE NULL END FROM worker_objective_records_legacy AS objectives JOIN tasks ON tasks.task_id = objectives.task_id JOIN provider_attempts AS attempts ON attempts.attempt_id = objectives.provider_attempt_id AND attempts.task_id = objectives.task_id JOIN implementation_attempts AS implementations ON implementations.provider_attempt_id = objectives.provider_attempt_id AND implementations.task_id = objectives.task_id LEFT JOIN implementation_candidates AS candidates ON candidates.implementation_attempt_id = implementations.implementation_attempt_id AND candidates.task_id = objectives.task_id LEFT JOIN provider_completion_outputs AS outputs ON outputs.attempt_id = objectives.provider_attempt_id",
            "INSERT INTO worker_objective_records(objective_id, task_id, dispatch_sha, provider_attempt_id, retry_identity, objective_digest, state, candidate_sha, completion_evidence_fingerprint, accepted_result_identity, terminal_reason_digest) SELECT objectives.objective_id, objectives.task_id, CASE WHEN implementations.repair_candidate_sha IS NULL THEN tasks.base_sha ELSE implementations.repair_candidate_sha END, objectives.provider_attempt_id, objectives.retry_identity, objectives.objective_digest, objectives.state, CASE WHEN objectives.state = 'completed' THEN candidates.candidate_sha ELSE NULL END, CASE WHEN objectives.state = 'completed' THEN attempts.completion_evidence_fingerprint ELSE NULL END, CASE WHEN objectives.state = 'completed' THEN outputs.output_fingerprint ELSE NULL END, CASE WHEN objectives.state = 'cancelled' THEN objectives.terminal_reason_digest ELSE NULL END FROM worker_objectives AS objectives JOIN tasks ON tasks.task_id = objectives.task_id JOIN provider_attempts AS attempts ON attempts.attempt_id = objectives.provider_attempt_id AND attempts.task_id = objectives.task_id JOIN implementation_attempts AS implementations ON implementations.provider_attempt_id = objectives.provider_attempt_id AND implementations.task_id = objectives.task_id LEFT JOIN implementation_candidates AS candidates ON candidates.implementation_attempt_id = implementations.implementation_attempt_id AND candidates.task_id = objectives.task_id LEFT JOIN provider_completion_outputs AS outputs ON outputs.attempt_id = objectives.provider_attempt_id",
            "DROP TABLE worker_objective_records_legacy",
