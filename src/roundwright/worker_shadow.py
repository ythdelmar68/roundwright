...
from .role_capability_policy import RoleCapabilityError, RoleExecutionSeam, SealedRoleExecution, TrustedRoleEffectReservation
...
def qualify_worker_adapter(adapter: CodexWorkerAdapter, request: CodexWorkerRequest, readiness: WorkerShadowCaptureReadiness, binding: WorkerQualificationBinding, recorder: ExternalWorkerRecorder, advisory_execution: SealedRoleExecution, effect_reservation: TrustedRoleEffectReservation, *, checkpoint_session: Callable[[str], None], checkpoint_turn: Callable[[str, str], None], checkpoint_result: Callable[[str, str, WorkerResultKind, WorkerParserDiagnostic | None, WorkerOutcomeSource | None, WorkerSdkTurnErrorCategory | None], None]) -> WorkerQualificationResult:
    ...
    try:
        effect_reservation.require_before_effect(
            advisory_execution, profile=adapter._profile,
        ...
        raise WorkerShadowError("Worker qualification effect reservation is invalid") from error
    try:
