...
class CiVerificationTests(unittest.TestCase):
    ...
    def test_docker_consumer_workflow_qualifies_the_uploaded_wheel_without_publication(self) -> None:
        ...
        self.assertIn('sudo find "$fixtures/state" -type d -exec chmod 0750 {} +', workflow)
        self.assertIn('sudo find "$fixtures/state" -type f -exec chmod 0640 {} +', workflow)
        self.assertIn('expect_blocked "state mount: missing"', workflow)
        self.assertIn('expect_blocked "state mount: permission-mismatch"', workflow)
        ...
        self.assertIn("result: blocked \\(((authentication|authority-receipt|configuration|repository|state) mount is", workflow)
        self.assertIn("authority inputs conflict)\\))$", workflow)
        self.assertNotIn("authority inputs conflict)$'; then", workflow)
