...
class SupervisorTests(unittest.TestCase):
    ...
    def test_later_schema_valid_fallback_is_accepted_after_invalid_primary(self):
        ...
        result = dispatch_ordered_supervisor_attempts((self.request(1, primary), self.request(2, fallback)), (primary, fallback), self.admissions((primary, fallback)), self.expectations((primary, fallback)), checkpoint_session=lambda identity: self.events.append(("session", identity)), checkpoint_turn=lambda session, turn: self.events.append(("turn", session, turn)))
    ...
    def test_omitted_expected_execution_prevents_any_backend_effect(self):
        ...
        with self.assertRaises(TypeError):
            adapter.dispatch(self.request(1, adapter), checkpoint_session=lambda identity: self.events.append(("session", identity)), checkpoint_turn=lambda session, turn: self.events.append(("turn", session, turn)), advisory_execution=self.admission(adapter))
    ...
    def test_exact_terminal_failure_advances_to_the_next_prebound_profile(self):
        ...
        result = dispatch_ordered_supervisor_attempts((self.request(1, primary), self.request(2, fallback)), (primary, fallback), self.admissions((primary, fallback)), self.expectations((primary, fallback)), checkpoint_session=lambda _identity: None, checkpoint_turn=lambda _session, _turn: None)
    ...
    def test_exhaustion_is_only_for_all_retryable_results_and_never_fabricates_a_verdict(self):
        ...
        result = dispatch_ordered_supervisor_attempts(tuple(self.request(index, adapter) for index, adapter in enumerate(adapters, start=1)), adapters, self.admissions(adapters), self.expectations(adapters), checkpoint_session=lambda _identity: None, checkpoint_turn=lambda _session, _turn: None)
    ...
    def test_terminal_failure_category_is_bound_into_the_public_result_identity(self):
        ...
        for category in (SupervisorSdkTurnErrorCategory.OVERLOAD, SupervisorSdkTurnErrorCategory.CONNECTION):
            ...
            result = adapter.dispatch(request, checkpoint_session=lambda _identity: None, checkpoint_turn=lambda _session, _turn: None, advisory_execution=self.admission(adapter), expected_execution=independent_execution(AdvisoryRole.SUPERVISOR, adapter._profile))
    ...
    def test_native_stream_rejections_only_advance_typed_invalid_output(self):
        ...
        for name, values, kind, diagnostic, fallback_calls in cases:
            with self.subTest(name=name):
                ...
                rejected = primary.dispatch(first, checkpoint_session=lambda _identity: None, checkpoint_turn=lambda _session, _turn: None, advisory_execution=self.admission(primary), expected_execution=independent_execution(AdvisoryRole.SUPERVISOR, primary._profile))
                ...
                ordered = dispatch_ordered_supervisor_attempts((first, self.request(2, fallback)), (primary, fallback), self.admissions((primary, fallback)), self.expectations((primary, fallback)), checkpoint_session=lambda _identity: None, checkpoint_turn=lambda _session, _turn: None)
    ...
    def test_sequence_fails_closed_when_durable_lifecycle_disagrees_with_provider_events(self):
        ...
        with self.assertRaisesRegex(Exception, "read-back failed"):
            qualify_supervisor_sequence(adapters, requests, self.admissions(adapters), self.expectations(adapters), readiness, binding, policy, lifecycle, recorder, evidence_time=101, freshness_until=120, runtime_store=self.runtime_store(), trusted_policy_receipt=self.trusted_receipt(binding, policy, readiness), review_authority_expectation=self.authority_expectation, review_authority_store=self.authority_store, review_authority_evidence=self.authority_evidence, checkpoint_session=lambda _identity: None, checkpoint_turn=lambda _session, _turn: None)
