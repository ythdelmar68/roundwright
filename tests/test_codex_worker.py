...
class CodexWorkerAdapterTests(unittest.TestCase):
    ...
    def dispatch(self, adapter, request, events):
        return adapter.dispatch(request, checkpoint_session=lambda session: events.append(f"session:{session}"), checkpoint_turn=lambda session, turn: events.append(f"turn:{session}:{turn}"), advisory_execution=sealed_execution(AdvisoryRole.WORKER, adapter._profile), expected_execution=independent_execution(AdvisoryRole.WORKER, adapter._profile))
    ...
    def test_omitted_expected_execution_prevents_any_provider_effect(self) -> None:
        events: list[str] = []
        ...
        with self.assertRaises(TypeError):
            adapter.dispatch(self.request(), checkpoint_session=lambda _: events.append("session"), checkpoint_turn=lambda *_: events.append("turn"), advisory_execution=sealed_execution(AdvisoryRole.WORKER, adapter._profile))
        self.assertEqual(events, [])
    ...
    def test_each_post_tool_sdk_turn_is_checkpointed_before_consumption(self) -> None:
        ...
        result = adapter.dispatch(
            ...
            expected_execution=independent_execution(AdvisoryRole.WORKER, adapter._profile),
        )
    ...
    def test_checkpoint_failure_never_consumes_output(self) -> None:
        ...
        turn = FakeTurn("turn-43", NativeWorkerResponse(WorkerResultKind.ACCEPTED, {"status": "done"}), events)
        adapter = self.adapter(FakeBackend(FakeSession("thread-43", turn, events)), events)
        result = adapter.dispatch(self.request(), checkpoint_session=lambda _session: None, checkpoint_turn=lambda _session, _turn: (_ for _ in ()).throw(RuntimeError("storage unavailable")), advisory_execution=sealed_execution(AdvisoryRole.WORKER, adapter._profile), expected_execution=independent_execution(AdvisoryRole.WORKER, adapter._profile))
    ...
    def test_session_checkpoint_failure_closes_without_starting_a_turn(self) -> None:
        ...
        turn = FakeTurn("turn-43", NativeWorkerResponse(WorkerResultKind.ACCEPTED, {"status": "done"}), events)
        adapter = self.adapter(FakeBackend(FakeSession("thread-43", turn, events)), events)
        result = adapter.dispatch(self.request(), checkpoint_session=lambda _session: (_ for _ in ()).throw(RuntimeError("storage unavailable")), checkpoint_turn=lambda _session, _turn: None, advisory_execution=sealed_execution(AdvisoryRole.WORKER, adapter._profile), expected_execution=independent_execution(AdvisoryRole.WORKER, adapter._profile))
    ...
    def test_out_of_order_step_is_ambiguous_without_callback(self) -> None:
        ...
        adapter=self.adapter(FakeBackend(FakeSession("thread-43",turn,events)),events); result=adapter.dispatch(request, checkpoint_session=lambda _:None, checkpoint_turn=lambda *_:None, execute_tool_request=lambda _: self.fail("callback"), advisory_execution=sealed_execution(AdvisoryRole.WORKER, adapter._profile), expected_execution=independent_execution(AdvisoryRole.WORKER, adapter._profile))
