...
def mounts(status: DockerMountStatus = DockerMountStatus.READY) -> tuple[DockerMountCheck, ...]:
    return tuple(DockerMountCheck(name, status) for name in DockerMountName)
...
class DockerConsumerTests(unittest.TestCase):
    def contract(self, **changes: object) -> DockerConsumerContract:
        values: dict[str, object] = {
            ...
            "authority_receipt_matches_candidate": True,
        }
    ...
    def test_authoritative_mode_requires_complete_candidate_bound_inputs(self) -> None:
        ...
        self.assertFalse(evaluate_docker_consumer(self.contract(authority_receipt_matches_candidate=False)).ready)
    ...
    def test_read_only_and_test_only_reject_authority_ambiguity(self) -> None:
        for mode in (DockerOperationMode.READ_ONLY, DockerOperationMode.TEST_ONLY):
            clean = self.contract(mode=mode, authority_receipt_digest=None, authority_receipt_matches_candidate=False)
    ...
    def test_cli_doctor_accepts_a_path_free_contract(self) -> None:
        arguments = [
            ...
            "--docker-package-digest", digest("b"), "--docker-base-image-digest", digest("c"),
            "--docker-authority-receipt-digest", digest("d"), "--docker-authority-receipt-matches-candidate",
        ]
    ...
    def test_dockerfile_uses_a_pinned_base_and_one_offline_wheel(self) -> None:
        ...
        self.assertIn("COPY --chown=65532:65532 dist/${ROUNDWRIGHT_WHEEL}", dockerfile)
