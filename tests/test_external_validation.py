...
class ExternalValidationTests(unittest.TestCase):
    ...
    def cross_environment_v2_request(self) -> tuple[external_validation.CrossEnvironmentCanaryInputs, dict[str, object], dict[str, object]]:
        ...
        plan: dict[str, object] = {
            ...
            "case_id": "issue-97-cross-environment", "candidate_sha": "c" * 40, "ready_at": 29,
            "producer_identity": producer, "exporter_identity": exporter, "comparator_identity": comparator,
            "recorder_identity": "sha256:" + "7" * 64, "store_identity": "sha256:" + "8" * 64,
            "observation_identity": "sha256:" + "9" * 64,
        }
        ...
        lanes = tuple(
            EnvironmentLane(
                ...
                sealed_canary_receipt_digest="sha256:" + "5" * 64,
            )
        ...
        sealed = SealedCanaryReceipt(
            "c" * 40, "sha256:" + "1" * 64, "sha256:" + "5" * 64,
            "sha256:" + "a" * 64, "sha256:" + "b" * 64, "sha256:" + "d" * 64,
            "ythdelmar68/roundlet-forward-test", "e" * 40, 29,
        )
        inputs = external_validation.CrossEnvironmentCanaryInputs(
            "c" * 40, "issue-97-cross-environment", plan_digest, 29,
    ...
    def test_cross_environment_profile_requires_one_bound_synthetic_matrix_and_zero_actions(self) -> None:
        ...
        prepared = adapter.prepare_execution_context(SimpleNamespace(
            ...
            plan=SimpleNamespace(plan_digest=external_validation._digest(plan), candidate_sha="c" * 40, case_id="issue-97-cross-environment", ready_at=29),
            components=adapter.component_identities,
        ))
        exact = binding(
    ...
    def test_integrated_hosted_entrypoint_rejects_mutated_selection_pins_before_dispatch(self) -> None:
        producer, exporter, comparator = external_validation.integrated_boundary_component_identities()
        plan = {
            "profile": INTEGRATED_BOUNDARY_PROFILE, "case_id": "issue-50-selection-pin-adversarial", "candidate_sha": "b" * 40,
            "ready_at": 17, "producer_identity": producer, "exporter_identity": exporter, "comparator_identity": comparator,
        }
        harness = sys.modules["roundwright_harness.executor"]
        before = len(harness.run_calls)
        for attribute in ("retention_manifest_digest", "lane_b_qualification_digest"):
            ...
            object.__setattr__(inputs.expectation, attribute, "sha256:" + "0" * 64)
            with self.subTest(attribute=attribute), self.assertRaises(external_validation.ExternalValidationAdapterError):
                external_validation.run_integrated_boundary_composition_profile("validate", request, Path("store"), inputs)
        self.assertEqual(len(harness.run_calls), before)
