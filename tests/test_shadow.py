...
class ShadowTests(unittest.TestCase):
    def identity(self) -> ShadowIdentity:
        return ShadowIdentity(
            ...
            "reference-38", (hashlib.sha256(b"input-38").hexdigest(),), "rules-38", "fixture-38", "2030-01-01T00:00:00Z", "phase-3", "retention-38", "normalizer-v1", "comparator-v1", ("input-38",), hashlib.sha256(b"reference-38").hexdigest(), (b"input-38",), b"reference-38", "sha256:" + "c" * 64,
        )
    ...
    def observations(self, **last_changes: object) -> tuple[ShadowObservation, ...]:
        ...
        for index, (state, role) in enumerate(zip(STATES, roles, strict=True), start=1):
            items.append(ShadowObservation(
                ...
                input_identities=("input-38",), input_digests=(hashlib.sha256(b"input-38").hexdigest(),), reference_result_digest=hashlib.sha256(b"reference-38").hexdigest(), input_payloads=(b"input-38",), reference_result_payload=b"reference-38", configuration_digest="sha256:" + "c" * 64,
            ))
